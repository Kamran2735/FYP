...
async function updateVariableTable(responseArray) {
  ...
  // Check if outputField and variableTableBody exist
  if (!outputField || !variableTableBody) {
    console.error("Output field or variable table body not found");
    return;
  }
  ...
  for (let i = 0; i < responseArray.length; i++) {
    ...
    if (response.error) {
      ...
      outputField.textContent = response.error;
